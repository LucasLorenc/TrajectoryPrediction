...
def eval(model_name, model_path='data/model', in_frames=8, out_frames=15, diff_fn=get_diff_array_v2, normalize=False,
    ...
    if evaluate:
        ...
        if log_var is not None:
            test_ll = -0.5 * np.exp(-log_var) * (pred - test_y) ** 2. - 0.5 * log_var - 0.5 * np.log(2 * np.pi)
            test_ll = np.sum(np.sum(np.sum(test_ll, -1), -1), -1)
            test_ll = logsumexp(test_ll) - np.log(mc_samples)
            test_ll = (test_ll / test_y.shape[0]) / test_y.shape[1]
            print("Test log liketralihood: " + str(-test_ll))
