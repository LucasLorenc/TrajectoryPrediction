...
from utils import *
from sequenceodo import SequenceOdo
...
def train(model_name, model_path='data/model', in_frames=8, out_frames=15, diff_fn=get_diff_array_v2, normalize=False,
    ...
    if force_load_data:
        ...
        _, _, _, image_sequence_paths_test, odometry_x_test, odometry_y_test, _ = test_dataset
        ...
        _, _, _, image_sequence_paths_train, odometry_x_train, odometry_y_train, _ = train_dataset
        save_pickle(dataset_pickle_path_train, train_dataset)

    else:
        _, _, _, image_sequence_paths_test, odometry_x_test, odometry_y_test, _ \
            = load_pickle(dataset_pickle_path_test)
        _, _, _, image_sequence_paths_train, odometry_x_train, odometry_y_train, _ \
    ...
    model_kwargs['model_input_shape'] = odometry_x_train.shape[1:]
    model_kwargs['model_visual_input_shape'] = [150, 300, 3]
    model_kwargs['model_output_dim'] = odometry_y_train.shape[-1]
    ...
    train_pred_gen =  SequenceOdo(odometry_x_train.copy(), odometry_y_train.copy(),
                                  image_sequence_paths_train.copy() if use_visual_features else None,
    ...
    if train_model:
        ...
        val_size = int(odometry_x_train.shape[0] * (1 - validation_split))

        val_x = odometry_x_train[val_size:]
        val_image_sequence_paths = image_sequence_paths_train[val_size:]
        val_y = odometry_y_train[val_size:]

        train_x = odometry_x_train[:val_size]
        train_image_sequence_paths = image_sequence_paths_train[:val_size]
        train_y = odometry_y_train[:val_size]

        test_x = odometry_x_test
        test_y = odometry_y_test
        ...
        train_gen = SequenceOdo(train_x, train_y, train_image_sequence_paths if use_visual_features else None,
                                batch_size=batch_size)
        val_gen = SequenceOdo(val_x, val_y, val_image_sequence_paths if use_visual_features else  None,
                              batch_size=batch_size)
    ...
    if evaluate:
        ...
        pred, log_var = predict(model, train_pred_gen, predict_var=predict_variance, mc_samples=mc_samples)
        save_pickle(odometry_pred_train, pred.mean(axis=0))

        test_gen = SequenceOdo(test_x, test_y,
                               image_sequence_paths_test if use_visual_features else None,
                               batch_size=batch_size, shuffle=False)
        pred, log_var = predict(model, test_gen, predict_var=predict_variance, mc_samples=mc_samples)
        save_pickle(odometry_pred_test, pred.mean(axis=0))

        if log_var is not None:
            aletoric = log_var.mean(axis=0)

        if normalize:
            test_y[:, :, 0] = inverse_standardization(test_y[:, :, 0], speed_mean, speed_std)
            test_y[:, :, 1] = inverse_standardization(test_y[:, :, 1], steer_mean, steer_std)
            ...
            if log_var is not None:
                ...
                aletoric = aletoric[:, :, 0] * speed_std**2
                aletoric = aletoric[:, :, 1] * steer_std**2
